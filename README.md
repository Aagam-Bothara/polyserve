...
- The later search stages tune every engine within 10% of the leader after the batch stage; one further behind is never tried with the options that later helped the leader. At 8 users a p95 rests on 32 requests, so its second-slowest can decide; a level too close to its ceiling to call is now measured again with as many requests, which has not yet run on a GPU. Only two workloads have been calibrated at p95, and the budget's new order has not been measured with the current warm-up.
