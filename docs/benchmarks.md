...
- **The quality gate was inert for this run.** `--max-quality-loss 0.02` was passed and recorded in the profile, but the probe was never built — the guard tested prompts that a file workload only materialises later, per concurrency level — so no precision was ever compared and the 4-bit pick was gated by nothing. The bug was fixed afterwards; the GSM8K numbers above are the only quality evidence behind this pick. A working gate would also not have caught it: 16 Dolly prompts measure whether instruction-following answers change, not whether multi-step arithmetic degrades.
