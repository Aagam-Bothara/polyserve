...
9. **A calibration judged at p95.** The [p95 numbers](#judged-at-the-95th-percentile) are re-scored from runs that picked by the median. The p95 rule, tuning every engine within 10% of the leader, the budget's new order, measuring the busiest level first and the confirmation stage have run in tests and replays only, and vLLM's GPU n-gram, suffix decoding, int8 KV cache and 8-bit W8A8 checkpoints have not run at all.
