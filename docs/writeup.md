...
A full grid would be 144 launches × ~30 s (engine startup dominates, not the 10 s workload) ≈ 70 minutes, and the variant stages below multiply it into the hundreds. The staged search runs 8–14 trials on the synthetic presets and 16–35 on the real-text calibrations since. Its order matters where settings interact: at the same budget, random sampling of the same space matched it on an A40 and an A100 and beat it by 29% on an RTX 4090, and an explore stage of random draws after the stages did not change that, but trying speculative decoding first did, matching random search's best there in 43 minutes against 66, and holding the tie on the A40 and A100 in two-thirds of their calibration time ([benchmarks](benchmarks.md#llama-31-8b-on-four-gpus)). What the stages buy is the same answer on every run, every strategy tried at least once, and a profile that says what each change was worth:
...
Later stages vary the leader along one dimension at a time: speculative decoding first, then the prefill budget, the KV-cache type and prefix-cache flags (on workloads whose prompts share a prefix), and optionally power. [Search options](usage.md#search-options) lists them. Speculation goes first because the other settings pay differently with it on: on an RTX 4090 with Llama 3.1 8B the int8 cache and a 16k prefill budget did nothing alone, yet with the draft model they were part of a configuration 28% faster than a pick that had tuned them before choosing suffix decoding. So the other stages run once for each of the engine's two best speculation settings (no speculation counts as one), on its best configuration with that setting, and the combinations share their trials between the two. The stages run for the leader and for every other engine whose best configuration after the batch stage is within 10% of it, or further behind when it offers a strategy the leader's engine lacks, each on its own configuration: on Dolly-15k prompts SGLang was 2% behind vLLM at that point, and tuning only the leader never tried it with the fp8 cache. Engines offering more strategies are tuned first, and one that an engine tuned before it has left more than 10% behind, with nothing more to offer, is skipped: on that 4090 SGLang spent 11 of 66 minutes on stages that ended 15% behind vLLM with speculation. With `--budget` they run straight after stage 1, before the memory and batch sweeps, since on real text that is where the gains were. Changing one dimension at a time misses strategies that only pay together, and keeps changes that get in a later one's way, so a final stage first measures the leader with each adopted change undone, then takes every change that came within 5% of the leader it was measured against, forms the combinations (one value per dimension, applied to the leader before the variant stages), estimates each by adding its members' gains, and measures the best, eight trials in all. The undo step came from an ablation: on `extract` on an A40 the search adopted the fp8 KV cache (+19% on its own) and then a draft model on top of it, but the draft model without the fp8 cache ran 19% faster than that pick, and summed gains ranked that configuration below every pair. With the undo step, a rerun found it on its second undo trial (726 tok/s against 604 for the adopted pair) and served it at 731 tok/s, 66.5% ahead of stock vLLM. A last stage measures the best three configurations again and chooses on those runs alone, because the best of many noisy trials is flattered by the choosing: on Dolly-15k prompts one pick measured 603 tok/s in calibration and 547 when re-measured.
...
Every trial uses the same fixed workload — 16 prompts × ~256-token prefill × 128-token decode, `temperature 0`, `ignore_eos`, at concurrency 1, 4 and 8 — so numbers are comparable across backends and across machines. Each concurrency level gets fresh prompts. Replaying one level's prompts at the next lets the engine's prefix cache make every later prefill nearly free: an early version of the harness did that and read one configuration at 3638 tok/s that measures 1814 with fresh prompts. A workload's deliberately shared prefix (the `chat-system` and `rag-shared` presets) is kept across levels, since caching it is the behaviour being measured. Summary metrics come from the concurrency level with the highest throughput (the load the server would actually be run at); energy is total joules over total tokens across the levels measured. Under `throughput` and `balanced` the levels are measured busiest first and the trial stops at the first that meets the objective's limits, since throughput rises with load and a quieter level cannot then beat it; replayed over six recorded calibrations (115 trials), that changed no score or pick and skipped 35% of calibration time. The replay could not show one effect: a level measured first pays for kernels compiled on first use, so the warm-up now fills every slot of the busiest level before measuring (see [benchmarks](benchmarks.md#measuring-the-busiest-level-first)).
...
(config, concurrency level) measured; a trial that stopped at its busiest level contributes only that
level, which is also the load the pick is scored at. `polyserve fit` reports leave-one-trial-out MAPE for tok/s and TTFT
...
- One change at a time. The stages adopt a change only when it pays on its own, and the combination stage joins only changes that came close to the leader alone, so an interaction neither change shows alone is missed. Given the same time, random sampling of the same settings matched the staged search on an A40 and an A100 and found such an interaction on an RTX 4090 (a draft model with an int8 KV cache and a 16k prefill budget, 29% faster than the pick; [benchmarks](benchmarks.md#llama-31-8b-on-four-gpus)). Spending part of the budget on random draws around the leader (`--explore on`) did not fix it; trying speculative decoding first and tuning the other settings on each of the two best speculation methods did, on that card (1041 tok/s against random search's 1041, in 43 minutes against 66). On the A40 and A100 it kept the ties already there (506 against 505, and 1155 against 1160) with a third fewer trials. It catches settings that pay only with a speculation method; other interactions would still be missed.
