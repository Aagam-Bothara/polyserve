...
| `--explore on\|off` | `on` | After the other stages and before `--confirm`, spend about 30% more trials (at least 4) on configurations of the leading engine and precision drawn at random from everything calibration could try. The stages change one setting at a time and combine only changes that paid alone, so they miss settings that only pay together: on an RTX 4090, random sampling found a draft model with an int8 KV cache and a 16k prefill budget, 29% faster than the staged pick. With `--budget`, explore trials are skipped like any other once the budget is spent. |
