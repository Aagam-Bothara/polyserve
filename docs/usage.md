...
| `--max-quality-loss FRACTION` | unset | Gate weight precisions on whether they still answer the same way. While each precision's first engine is up, up to 48 of the calibration prompts (drawn across concurrency levels, since a workload materialises one level at a time) are answered greedily **twice**, and only the first 32 tokens of each answer are compared: greedy decoding diverges once a single token differs, so a whole 128-token completion disagrees for almost any quantization, while its opening says whether the model started the same answer. The second pass sets a noise floor — identical weights answering twice already disagree, because batched decoding is not bit-reproducible — and a precision is dropped only when it differs from the most faithful precision that runs by more than FRACTION *above that floor*. Drift moves in steps of one prompt, so the tolerance is coarser than it looks: over 48 prompts `0.02` means no changed answer at all beyond the noise floor, and `0.05` allows two. Pick it against the prompt count, not as a percentage in the abstract. It needs no labels: it measures drift on your own prompts, not accuracy, so it says "these weights answer differently", not "these weights are worse". Costs one short generation pass per precision, and the profile records what each precision drifted. Unset, quality never gates the search and is measured separately, against GSM8K, with the narrowness that implies ([what is and is not graded](benchmarks.md#not-yet-measured)). Two limits: this measures drift, not accuracy, so sixteen instruction-following prompts can agree perfectly while reasoning degrades — on a 14B at 4-bit they would have, where GSM8K lost about 2 points; and its first run on hardware built no probe at all, a bug fixed afterwards ([both](benchmarks.md#qwen25-14b-where-memory-binds)). |
